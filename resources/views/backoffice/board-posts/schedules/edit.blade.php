...
<div class="board-container">
    <div class="board-header">
        <a href="{{ route('backoffice.board-posts.index', $board->slug ?? 'schedules') }}" class="btn btn-secondary btn-sm">
    ...
    <div class="board-card">
        <div class="board-card-body">
            ...
            <form action="{{ route('backoffice.board-posts.update', [$board->slug ?? 'schedules', $post->id]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="board-form-group">
                    ...
                    <textarea class="board-form-control board-form-textarea summernote-editor" id="content" name="content" rows="15" @if($board->isFieldRequired('content')) required @endif>{{ old('content', $post->content) }}</textarea>
                </div>
                @endif
                        ...
                        @if($fieldConfig['type'] === 'display_date_range')
                            ...
                            <div class="board-form-group display-date-range-selector" data-field-name="{{ $fieldConfig['name'] }}">
                                ...
                                <input type="hidden" 
                                       id="custom_field_{{ $fieldConfig['name'] }}" 
                                       name="custom_field_{{ $fieldConfig['name'] }}" 
                                       value="{{ old('custom_field_' . $fieldConfig['name'], json_encode(['use_display_date' => true, 'start_date' => $startDate, 'end_date' => $endDate])) }}">
                            </div>
                        @endif
                    @endforeach
                @endif
                ...
                <div class="board-form-actions">
                    ...
                    <a href="{{ route('backoffice.board-posts.index', $board->slug ?? 'schedules') }}" class="btn btn-secondary">취소</a>
                </div>
            </form>
        </div>
    </div>
