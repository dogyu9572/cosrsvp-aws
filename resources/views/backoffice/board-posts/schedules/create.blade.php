        ...
        <div class="board-card-body">
            ...
            <form action="{{ route('backoffice.board-posts.store', $board->slug ?? 'top-notices') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="board-form-group">
                    ...
                    <textarea class="board-form-control board-form-textarea summernote-editor" id="content" name="content" rows="15" @if($board->isFieldRequired('content')) required @endif>{{ old('content') }}</textarea>
                </div>
                @endif
                ...
                @if($board->custom_fields_config && count($board->custom_fields_config) > 0)
                    @foreach($board->custom_fields_config as $fieldConfig)
                        @if($fieldConfig['type'] === 'display_date_range')
                            @php
                                $displayDateData = old('custom_field_' . $fieldConfig['name']) ? json_decode(old('custom_field_' . $fieldConfig['name']), true) : ['use_display_date' => true, 'start_date' => '', 'end_date' => ''];
